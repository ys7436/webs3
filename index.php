...
<div id="touch-section2" class="section section2" style="background:url(img/bg3.jpg) no-repeat center;background-size:cover;">
    ...
    <div class="section form" action="/form.php" style="background:url(img/bg4.jpg) no-repeat top center;background-size:cover;">
       ...
       <div class="operation">
            <div class="clearfix">
                <div class="hover">
                    <div>
                        <!-- <button id="tabMengpi-button">切换</button> -->
                        <p style="color:#000000;font-family:'Microsoft YaHei'">了解详情</p>
                        <img src="img/more01.png">
                    </div>
                </div>
...
<script type="text/javascript" src="js/ios/uploadImage.js" ></script>  
...
<script>
    ...
    function iosOrientReset(source, canvas, tempContext)
    {
        var orient = getPhotoOrientation(source);

        if (orient == 6) {
          // iosReset=true;

          tempContext.save(); //保存状态
          tempContext.translate(canvas.width, 0); //设置画布上的(0,0)位置，也就是旋转的中心点
          tempContext.rotate(90 * Math.PI / 180); //把画布旋转90度
          // 执行Canvas的drawImage语句

          // img.src = document.getElementById('uplos').src = photo.src; // 设置图片源地址

          tempContext.drawImage(source,imgout[0], imgout[1], imgout[2], imgout[3], 0, 0, canvas.width+canvas.width, canvas.height+canvas.height);//把图片绘制在画布translate之前的中心点，
          // tempContext.drawImage(source, imgout[0], imgout[1], imgout[2], imgout[3], 0 - canvas.height/2, 0 - canvas.width/2, canvas.width, canvas.height);//把图片绘制在画布translate之前的中心点，


          tempContext.restore(); //恢复状态
        } else if(orient == 3) {

            tempContext.save(); //保存状态
            tempContext.translate(canvas.width, canvas.height); //设置画布上的(0,0)位置，也就是旋转的中心点
            tempContext.rotate(180 * Math.PI / 180); //把画布旋转90度
            // 执行Canvas的drawImage语句

            // img.src = document.getElementById('uplos').src = photo.src; // 设置图片源地址

            tempContext.drawImage(source,imgout[0], imgout[1], imgout[2], imgout[3], 0, 0, canvas.width, canvas.height);//把图片绘制在画布translate之前的中心点，
        ...
        function previewImage(file,id){
            ...
            //imageupload.onload = function(){
				...
				//获取照片方向角属性，用户旋转控制  
				Orientation = getPhotoOrientation(source);
                ...
                } else if (this.naturalHeight > this.naturalWidth && this.naturalHeight > 1165) {  
                    expectHeight = 1165;  
                ...
                rotateCanvas.width = expectWidth;  
                rotateCanvas.height = expectHeight;  
                rotateContext.drawImage(this, 0, 0, expectWidth, expectHeight);  
                var base64 = null;  
                //修复ios  
                if (navigator.userAgent.match(/iphone/i)) {  
                    ...
                    if(Orientation != "" && Orientation != 1){  
                        ...
                        switch(Orientation){  
                            case 6://需要顺时针（向左）90度旋转  
                                ...
                                rotateImg(this,'left',rotateCanvas);  
                                break;  
                            case 8://需要逆时针（向右）90度旋转  
                                //alert('需要顺时针（向右）90度旋转');  
                                rotateImg(this,'right',rotateCanvas);  
                                break;  
                            case 3://需要180度旋转  
                                //alert('需要180度旋转');  
                                rotateImg(this,'left2',rotateCanvas);//转两次  
                                //rotateImg(this,'right',rotateCanvas);  
                                break;  
                        }         
                    }  
                    ...
                    base64 = rotateCanvas.toDataURL("image/jpeg", 0.8);  
                }else if (navigator.userAgent.match(/Android/i)) {// 修复android  
                    var encoder = new JPEGEncoder();  
                    base64 = encoder.encode(rotateContext.getImageData(0, 0, expectWidth, expectHeight), 80);  
                }else{  
                    //alert(Orientation);  
                    if(Orientation != "" && Orientation != 1){  
                        //alert('旋转处理');  
                        switch(Orientation){  
                            ...
                            case 3://需要180度旋转  
                                //alert('需要180度旋转');  
                                rotateImg(this,'right2',rotateCanvas);//转两次  
                                //(this,'right',rotateCanvas);  
                                break;  
                        }         
                    }  
				...
				imageupload.onload = function(){
					...
					tempContext.drawImage(source, imgout[0], imgout[1], imgout[2], imgout[3],0,0, canvas.width, canvas.height);
					//iosOrientReset(source, canvas, tempContext);
					adjustColor2();
...
/*
		...
		*/
		//增加点击是见
        $(document).on("click","#uplos", function(e){
        ...
        $(document).on("click","#pan-img", function(e){
			e.preventDefault();
			ljIndex++; if (ljIndex==5) {ljIndex=0};
			getImageDrawWord("imageupload-result"+(ljIndex), getCurrentQh())
