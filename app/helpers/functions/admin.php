...
function uppro($name,&$arr,$style='img',$water_path=''){
    ...
    $path = PUBLIC_PATH . Config::get('pic.upload');
...
function printImgHtml($lablename,$imgname,$ty,$type=0){
    ...
    if ($type) {//文件
        ...
        if ($$imgname) {
            $thisimg = src($$imgname);
            $IMG = '<input name="'.$imgname.'" type="file"> <a target="_blank" href="'.$thisimg.'"><img src="'.$thisimg.'" height="40" /></a><a href="'.$thisimg.'" target="_blank">查看图片</a>';
        }else{
            $IMG = '<input name="'.$imgname.'" type="file">';
        }
    }
